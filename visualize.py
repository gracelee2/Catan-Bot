...
class Visualize:
    ...
    def playGame(self):
        ...
        board_width = 15 * 70  # Default width based on 70px tiles
        board_height = 15 * 70  # Default height based on 70px tiles

        # Calculate scale factor
        scale_factor = min(screen_width / board_width, screen_height / board_height) * 0.92

        # Adjust tile size dynamically
        w = 70 * scale_factor
        self.tile_size = w  # Store scaled tile size for later use

        # Create scaled canvas
        c = Canvas(root, width=board_width * scale_factor, height=board_height * scale_factor, background='#ADD8E6')
        self.c = c
        c.pack()

        center = [w * 7.5, w * 6]  # Center of the board scaled
        ...
        for port in threeOnes:
            ...
            c.create_text(x,y,fill='black',font=f"Times {int(25 * scale_factor)} bold",
                          text='3:1')
        ...
        turn_label = c.create_text(100,50,fill='black', font =f"Times {int(25 * scale_factor)} bold",
                                text = self.game.current_player.name+"'s Turn")

        #Top right tell you the current players point tally
        point_counter = c.create_text(950,50,fill='black', font = f"Times {int(25 * scale_factor)} bold",
                                    text = 'Points: '+ \
        ...
        self.devs['monopoly'] = c.create_text(900,910,fill='black',
                        font=f"Times {int(25 * scale_factor)} bold", text='Monopoly x '+ \
        ...
        self.devs['road_builder'] = c.create_text(900,945,fill='black',
                        font=f"Times {int(25 * scale_factor)} bold",text='Road Builder x '+ \
        ...
        self.devs['year_of_plenty'] = c.create_text(900,980,fill='black',
                        font=f"Times {int(25 * scale_factor)} bold",text='Year of Plenty x '+ \
        ...
        self.devs['point_cards'] = c.create_text(900,1015,fill='black',
                        font=f"Times {int(25 * scale_factor)} bold", text='Point Card x '+ \
        ...
        def nextTurn():
            ...
            if self.game.dieRoll == 7:
                if isinstance(self.game.current_player, Robot):
                    catan_tiles = [
                    (6, 'wood'), (8, 'brick'), (2, 'grain'), (5, 'brick'),
                    (4, 'wood'), (5, 'ore'), (9, 'grain'), (4, 'brick'),
                    (9, 'sheep'), (11, 'wood'), (10, 'sheep'), (3, 'wood'),
                    (6, 'ore'), (8, 'grain'), (12, 'sheep'), (3, 'ore'),
                    (10, 'sheep'), (11, 'grain')
                ]
                    random_tile = random.choice(catan_tiles)
                    print(f"Random tile chosen: Number {random_tile[0]} with resource {random_tile[1]}")
        ...
        tmp = c.create_text(980,995,fill='black',font = f"Times {int(19 * scale_factor)} bold",
                            text = 'END TURN')
        c.tag_bind(tmp,"<Enter>", lambda event,
        ...
        dev_box = c.create_rectangle(915,965,1045,915,
                                     fill = '#A9A9A9')
        tmp = c.create_text(980,940,fill='black',font = f"Times {int(19 * scale_factor)} bold",
                            text = 'BUY DEV-C')
        c.tag_bind(tmp,"<Enter>", lambda event,
        ...
        tmp = c.create_text(980,885,fill='black',font = f"Times {int(19 * scale_factor)} bold",
                            text = 'TRADE IN')
        c.tag_bind(tmp,"<Enter>", lambda event,
        ...
        tmp = c.create_text(980,830,fill='black',font = f"Times {int(19 * scale_factor)} bold",
                            text = '  TRADE \nPLAYERS')
        c.tag_bind(tmp,"<Enter>", lambda event,
        ...
        if isinstance(self.game.current_player, Robot) and not self.freeze:
            ...
            try:
                if action[0] == 'settlement':
                    # Simulate clicking a settlement
                    ...
                    self.c.event_generate('<Button-1>', x=int(x), y=int(y))
                elif action[0] == 'trade_in':
                    print("The Bot is trading in resources.")
                    most_resource = max(self.hand, key=self.hand.get)
                    least_resource = min(self.hand, key=self.hand.get)
                    print(f"Bot chooses to trade {least_resource} for {most_resource} ")
                    self.tradeWindow()
                    ...
                    if action[1] == 'knight':
                        ...
                        least_resource1 = min(self.hand, key=self.hand.get)
                        least_resource2 = min(self.hand, key=self.hand.get)
                        print(f"Bot chooses {least_resource1} and {least_resource2} as its Year of Plenty resource.")
                    else:
                        print("Invalid dev card action.")
                        return

                    # Calculate center of the button for accurate event generation
                    x = (coords[0] + coords[2]) / 2
                    y = (coords[1] + coords[3]) / 2
                    print(f"Simulating use of {action[1]} card at: ({x}, {y})")
                    self.c.event_generate('<Button-1>', x=int(x), y=int(y))
                else:
